...
export default function TwinCanvas3D({ machines, telemetryData, activeFlowIndex, onMachineHover }: TwinCanvas3DProps) {
  return (
    ...
    >
      ...
      {/* Scrap flow arrows - only between active machine and next */}
      <ScrapFlowArrows
        machines={machines}
...
function MachineModel({ machine, telemetry, position, index, isActive, onHover }: {
  ...
  useFrame((_, delta) => {
    if (glowRef.current) {
      // Machines always visible, but glow much brighter when active (scrap entering)
  ...
  // Base emissive intensity - always visible, brighter when active
  const baseEmissive = isActive ? 0.35 : 0.08
  const machineColor = '#2a3a4a' // Uniform visible color for all machines
...
function getHeight(type: string): number {
  if (type.includes('tank')) return 2.8
...
function MachineShape({ type, machineColor, emissiveColor, emissiveIntensity, isActive, onPointerOver, onPointerOut }: {
  ...
  // Conveyor - flat elongated box with rollers
  ...
  // Sorting machine - box with angled chutes and sensor
  if (type.includes('sorting')) {
    return (
      <group onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
  ...
  // Shredder - octagonal cylinder with hopper
  if (type.includes('shredder')) {
    return (
      <group onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
  ...
  // Magnetic separator - wide drum with coils
  if (type.includes('separator') || type.includes('magnetic')) {
    return (
      <group onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
        <mesh position={[0, 1, 0]} castShadow>
  ...
  // Chemical tank - tall cylinder with dome and pipes
  if (type.includes('tank') || type.includes('chemical')) {
    return (
      <group onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
  ...
  // Drying unit - box with vents and exhaust
  if (type.includes('drying') || type.includes('dry')) {
    return (
      <group onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
...
// Scrap flow arrows - animated arrow shapes that move from active machine to the next
function ScrapFlowArrows({ machines, activeFlowIndex }: { machines: MachineConfig[]; activeFlowIndex: number }) {
  ...
  // Calculate positions for arrows between active machine and next
  const getPositions = useCallback(() => {
    ...
    return { startX, endX }
  }, [activeFlowIndex, machines.length])
...
function FlowArrow({ startX, endX, delay }: { startX: number; endX: number; delay: number }) {
  ...
  useFrame(() => {
    ...
    ref.current.scale.setScalar(0.8 + opacity * 0.4)
  })

  return (
    <group ref={ref} rotation={[0, -Math.PI / 2, 0]}>
      {/* Arrow cone pointing in direction of flow */}
